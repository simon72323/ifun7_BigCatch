...
class PromotionalContent {
	...
	constructor(pmInstance) {
		...
		this.promoteInformation = this.promotionInstance.promotionInformation;
        this.jackpotInformation = this.promotionInstance.jackpotInformation;
    ...
    createPromotionWindow() {
        let url = './extensions/data/promotionalContent.html';
    ...
    createJackpotWindow() {
        let url = './extensions/data/jackpotContent.html';
        fetch(url).then( response => response.text() ).then((result) => {
		...
		switch (type) {
			case 'dropMultiplePlayerData':
				...
				for (let i = 0; i < this.promoteInformation[index].winner.length; i++) {
					content += '<tr>' +
						...
						'<td class="pd10 text-end">' + this.promoteInformation[index].winner[i].bet_credit.changeUnit() + '</td>' +
						'<td class="pd10 text-end">' + this.promoteInformation[index].winner[i].prize_credit.changeUnit() + '</td>' +
						'</tr>';
				}
			...
			case 'dropPlayerData':
				...
				for (let i = 0; i < this.promoteInformation[index].winner.length; i++) {
					content += '<tr>' +
						'<td>' + this.promoteInformation[index].winner[i].account + '</td>' +
						'<td class="pd200 text-end">' + this.promoteInformation[index].winner[i].prize_credit.changeUnit() + '</td>' +
						'</tr>';
				}
				for (let i = this.promoteInformation[index].winner.length; i < 20; i++) {
					content += '<tr>' +
			...
			case 'randomPrize':
				content = '<tr>' +
					...
					'<tr>' +
					'<td>Minimum</td>' +
					'<td class="pd200 text-end">' + this.promoteInformation[index].currency + ' ' + this.promoteInformation[index].mode_rule[0].changeUnit() + '</td>' +
					'</tr>';
				break;
			...
			case 'columnPrize':
				for (let i = 0; i < this.promoteInformation[index].mode_rule.length; i++) {
					content += '<tr>' +
						'<td>' + (i + 1) + '</td>' +
						'<td class="pd200 text-end">' + this.promoteInformation[index].currency + ' ' + this.promoteInformation[index].mode_rule[i].changeUnit() + '</td>' +
						'</tr>';
				}
				break;
			...
			case 'history':
				...
				for (let i = 0; i < this.promoteInformation[index].user.length; i++) {
					content += '<tr>' +
						...
						'<td class="pd200 text-end">' + this.promoteInformation[index].user[i].prize_credit.changeUnit() + '</td>' +
						'</tr>';
				}
				break;
				...
				for (let i = 0; i < this.promoteInformation[index].bonus_setting.length; i++) {
					if (this.promoteInformation[index].bonus_setting[i].start_rank === this.promoteInformation[index].bonus_setting[i].end_rank) {
						content += '<tr>' +
							...
							'<td class="pd200 text-end">' + this.promoteInformation[index].currency + ' ' + this.promoteInformation[index].bonus_setting[i].bonus.changeUnit() + '</td>' +
							'</tr>';
					} else {
						content += '<tr>' +
			...
			case 'tournamentPlayerData':
				...
				for (let i = 0; i < this.promoteInformation[index].winner.length; i++) {
					content += '<tr id="tournamentPlayer' + index + '-' + i + '">' +
						'<td>' + (i + 1) + '</td>' +
						'<td>' + this.promoteInformation[index].winner[i].account + '</td>' +
						'<td class="pd10 text-end">' + this.promoteInformation[index].winner[i].credit.changeUnit() + '</td>' +
						(this.promoteInformation[index].winner[i].prize_credit.changeUnit() === '0' ? '<td class="pd10"> - </td>' : '<td class="pd10 text-end">' + this.promoteInformation[index].winner[i].prize_credit.changeUnit() + '</td>') +
						'</tr>';
				}
				for (let i = this.promoteInformation[index].winner.length; i < 20; i++) {
					content += '<tr>' +
            ...
            case 'jackpotData':
                ...
                for (let i = 0; i < this.jackpotInformation[index].winner.length; i++) {
                    content += '<tr id="JackpotPlayer' + index + '-' + i + '">' +
                        ...
                        '<td>' + this.jackpotInformation[index].winner[i].prize_credit.changeUnit() + '</td>';
				}
	...
	createRulePage(i) {
		...
		this.$rule = $('<div class="col">' +
			'<p class="rule">' + this.promoteInformation[i].promotion_content + '</p>' +
			'</div>');
	...
	updatePromoteWinner() {
		...
		for (let i = 0; i < this.promoteInformation.length; i++) {
			...
			if (this.promoteInformation[i].user !== undefined) {
				if (this.promoteInformation[i].promotion_type === 0) {
					$('#userPrize' + i).html(this.promoteInformation[i].user[0] !== undefined ? this.promoteInformation[i].user[0].prize_credit.toFixedNoRound() : '_');
					...
					$('#userPrize' + i).html(this.promoteInformation[i].user.prize_credit.toFixedNoRound().changeUnit() === '0' ? '-' : this.promoteInformation[i].user.prize_credit.toFixedNoRound().changeUnit());
					$('#rank' + i).html((this.promoteInformation[i].user.rank + 1) > 500 ? '500+' : (this.promoteInformation[i].user.rank + 1));
					$('#credit' + i).html(this.promoteInformation[i].user.credit.changeUnit());
    ...
    updateJackpotWinner() {
        ...
        for (let i = 0; i < this.jackpotInformation.length; i++) {
            ...
            if (this.jackpotInformation[i].user !== undefined && this.jackpotInformation[i].user.length !== 0) {
                ...
                $('#myJpAmount' + i).html(this.jackpotInformation[i].user[0].prize_credit.changeUnit());
			}
		}
    }

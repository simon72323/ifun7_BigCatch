...
export class SlotReelMachine extends Component {
    private static instance: SlotReelMachine;

    public static getInstance(): SlotReelMachine {
        return SlotReelMachine.instance;
    }
    ...
    onLoad() {
        ...
        SlotReelMachine.backBSParser.on(this.onBackBSParser, this);
    ...
    private onBackBSParser(backBSParser: number[][]) {
        for(let i = 0; i < backBSParser.length; i++) {
            for(let j = 0; j < backBSParser[i].length; j++) {
                const symbol = this.reelSymbols[i][j];
                symbol.setSymbolID(backBSParser[i][j]);
            }
        }
    }
    ...
    private async onSlotRun(resultPattern: number[][], mipieList: boolean[]) {
        ...
        }, BaseConst.SLOT_TIME[DataManager.getInstance().curTurboMode].beginTime);
     ...
     * @param runTime 轉動時間e
     */
    private async stopSlotRun(reelIndex: number): Promise<void> {
        return new Promise(async resolve => {
            ...
            const { runTime, backTime } = this.handleMi(reelIndex);//判斷是否執行咪牌與回傳停止時間
            let isResolve = false;
    ...
    public onSlotSkip() {
        ...
        for (let i = 0; i < this.reelStopState.length; i++) {
            ...
            if (!this.reelStopState[i]) {
                ...
                this.stopSlotRun(i);//執行停止slot轉動
            }
            //瞇牌中的slot要停止並重跑停止方式
            if (this.mipieStopList[i]) {
                this.mipieStopList[i] = false;
                Tween.stopAllByTarget(this.reelList[i]);//停止該行動畫
                this.stopMipieSlotRun(i);
            }
    ...
    public static getSymbolList(): Node[] {
        return SlotReelMachine.instance.allMainSymbols;
    }
    ...
    public static getSymbolPosList(): Vec3[] {
        return SlotReelMachine.instance.allMainSymbolPos;
    }

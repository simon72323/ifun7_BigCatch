...
import { FreeSpinInfoBtn } from '../campaign/FreeSpinInfoBtn';
...
export class SettingsPage1 extends BaseSettingsPage {
    ...
    //長壓參數
    private betDelay: number = 0.3;
    private isButtonPressed: boolean = false;
    private buttonDirection: number = 0;
    private pressTimer: number = 0;
    private pressThreshold: number = 0.2;
    private minPressThreshold: number = 0.1;
    ...
    private onTouchPlus(): void {
        ...
        this.scheduleOnce(this.startPress, this.betDelay);
        //先註冊schedule再發送click事件, 如果bet改變後已經到底才有辦法立刻取消startPress
    ...
    private stopPress(): void {
        ...
        this.unschedule(this.startPress);
    ...
    update(deltaTime: number) {
        ...
        // 若未達到觸發門檻，則直接返回
        if (this.pressTimer < this.pressThreshold) return;

        // 重置計時器並降低門檻（最小不低於 0.1）
        this.pressTimer = 0;
        this.pressThreshold = Math.max(this.minPressThreshold, this.pressThreshold - 0.05);
    ...
    onLoad() {
        ...
        SettingsPage1.plusEnabled.on((enabled) => {
            ...
            if (enabled) {
                plusBtn.on(Input.EventType.TOUCH_START, this.onTouchPlus, this);
                plusBtn.on(Input.EventType.TOUCH_END, this.stopPress, this);
                plusBtn.on(Input.EventType.TOUCH_CANCEL, this.stopPress, this);
            }
            else {
                plusBtn.off(Input.EventType.TOUCH_START, this.onTouchPlus, this);
                plusBtn.off(Input.EventType.TOUCH_END, this.stopPress, this);
                plusBtn.off(Input.EventType.TOUCH_CANCEL, this.stopPress, this);
        ...
        SettingsPage1.lessEnabled.on((enabled) => {
            ...
            if (enabled) {
                lessBtn.on(Input.EventType.TOUCH_START, this.onTouchLess, this);
                lessBtn.on(Input.EventType.TOUCH_END, this.stopPress, this);
                lessBtn.on(Input.EventType.TOUCH_CANCEL, this.stopPress, this);
            }
            else {
                lessBtn.off(Input.EventType.TOUCH_START, this.onTouchLess, this);
                lessBtn.off(Input.EventType.TOUCH_END, this.stopPress, this);
                lessBtn.off(Input.EventType.TOUCH_CANCEL, this.stopPress, this);
    ...
    private setEnabled(enabled: boolean) {
        //免費轉中禁止使用任一按鈕
        enabled = enabled && this.isFreeSpin === false;

        //非自動轉才能禁用
        let autoEnabled: boolean = enabled || BaseDataManager.getInstance().auto.isAutoPlay();
        ...
        if (this.node.getChildByPath("MenuButton/Click")) {
            ...
            this.node.getChildByName("MenuButton").getComponent(Button).interactable = enabled;
        }

        FreeSpinInfoBtn.setEnabled.emit(enabled);
    }

import { _decorator, Component, director, EventKeyboard, game, input, Input, instantiate, KeyCode, Label, macro, Mask, Node, Prefab, screen, Sprite, SpriteFrame, sys, UITransform, View } from 'cc';
import { BaseConst } from '../../script/constant/BaseConst';
import { BaseDataManager } from '../../script/main/BaseDataManager';
import { BaseEvent } from '../../script/main/BaseEvent';
import { BundleLoader } from '../../script/main/BundleLoader';
import { BaseLang, BaseLangBundleDir } from '../../script/types/BaseType';
import { APIManager } from '../../script/utils/APIManager';
import { ErrorCode, ErrorManager } from '../../script/utils/ErrorManager';
import { TimeoutManager } from '../../script/utils/TimeoutManager';
import { logger } from '../../script/utils/XUtils';
import { Notice } from '../notice/Notice';
import { BaseGameLoading } from './BaseGameLoading';

const { ccclass, property } = _decorator;
@ccclass('LoadingScene')
export class LoadingScene extends Component {

    @property({ type: SpriteFrame, tooltip: "遊戲滿版背景" })
    public bgDesktop: SpriteFrame = null;

    @property({ type: SpriteFrame, tooltip: "遊戲模糊底圖" })
    public blurBg: SpriteFrame = null;
    ...
    async onLoad() {
        ...
        this.FakeLoad = this.node.getChildByPath("FakeLoad");
        this.FakeLoad.getComponent(Sprite).spriteFrame = this.blurBg;

        this.widthBg = this.node.getChildByPath("WidthBg");
        this.widthBg.getComponent(Sprite).spriteFrame = this.bgDesktop;
        ...
        if (sys.isMobile == true) {
            if ((rect.width / rect.height) > 720 / 1280) {
                this.GameVersion.setPosition((rect.width * (1280 / rect.height) / 2) - 20, -(rect.height * (1280 / rect.height) / 2));
            }
            else {
                this.GameVersion.setPosition((rect.width * (720 / rect.width) / 2) - 20, -(rect.height * (720 / rect.width) / 2));
            }
        }
        else {
            if (rect.width / rect.height > 720 / 1280) {
                if (rect.width / rect.height < 1) {
                    this.GameVersion.getComponent(Label).fontSize = 15;
                    this.GameVersion.setPosition((rect.width * (1280 / rect.height) / 2) - 10, -660);
                }
                else {
                    this.GameVersion.getComponent(Label).fontSize = 20;
                    this.GameVersion.setPosition((rect.width * (1280 / rect.height) / 2) - 10, -640);
                }
            }
            else {
                this.GameVersion.getComponent(Label).fontSize = 15;
                this.GameVersion.setPosition(350, -(rect.height * (720 / rect.width) / 2 + 40));
            }
        }
        ...
        View.instance.setResizeCallback(() => {
            ...
            if (sys.isMobile == true) {
                if ((rect.width / rect.height) > 720 / 1280) {
                    this.GameVersion.setPosition((rect.width * (1280 / rect.height) / 2) - 20, -(rect.height * (1280 / rect.height) / 2 + 100));
                }
                else {
                    this.GameVersion.setPosition((rect.width * (720 / rect.width) / 2) - 20, -(rect.height * (720 / rect.width) / 2 + 100));
                }
            }
            else {
                if (rect.width / rect.height > 720 / 1280) {
        ...
        BundleLoader.onLoaded(BaseConst.BUNDLE_BASE_LANGUAGE, `${BaseDataManager.getInstance().urlParam.lang}/${BaseLangBundleDir.loading}`, (langRes: any) => {
        ...
        //API版本
            ...
            (function () {
                ...
                (function (a) {
                    ...
                    var a = param.hasOwnProperty("origin") ? decodeURIComponent(param['origin']) : "";

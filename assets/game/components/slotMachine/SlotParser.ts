...
export class SlotParser extends BaseSlotParser {
     ...
     * @returns 
     */
    public getMiList2(fromMap: BaseSymbolData[][]): boolean[] {
        // let gameData = DataManager.getInstance().getData<GameData>();
        let miList: boolean[] = [];
        let scatterCount: number = 0;
        fromMap.forEach((symbolOfReel: BaseSymbolData[], col) => {
            scatterCount += symbolOfReel.filter((symbolData: BaseSymbolData) => symbolData.symbolID === SymbolID.Scatter).length;
        });
        ...
        if (scatterCount >= GameConst.BONUS_WIN_COUNT - 1) {
            ...
            fromMap.forEach((symbolOfReel: BaseSymbolData[], col) => {
                //該軸沒有sc就可以瞇牌
                miList.push(symbolOfReel.filter((symbolData: BaseSymbolData) => symbolData.symbolID === SymbolID.Scatter).length == 0);
            });
        }
        else {

...
export class GameData extends BaseData {
  ...
  public getTurboSetting(): TurboSetting {
    return this.turboSettings[BaseDataManager.getInstance().getTurboMode()];
  }
  ...
  public getGameHelpPayload(): any {
    ...
    for (let key in responsePayload.symbolPoints) {
      ...
      payDataList.forEach((data) => {
        data.point = XUtils.NumberToCentString(data.point * BaseDataManager.getInstance().bet.getCurTotal());
      }, this);
    }
    ...
    responsePayload.bet.featureLimit = BaseDataManager.getInstance().getFeatureBuyMaxBet().toString();
    responsePayload.bet.featureMultipleClassic = BaseDataManager.getInstance().getFeatureBuyMultipleByType(0);
  ...
  public getPayBySymbolID(symbolID: number): { count: number, cent: string }[] {
    ...
    if (payData) {
      payData.forEach((data) => {
        result.push({ count: data.count, cent: XUtils.NumberToCentString(data.point * BaseDataManager.getInstance().bet.getCurTotal()) });
      }, this);
    }
    return result;
  }
  ...
  public getInitMultiplierIdx(): number {
    return BaseDataManager.getInstance().isBS() ? 0 : GameConst.FS_INIT_MULTIPLIER_INDEX;
  }
  ...
  public getGoldenReelRange(): number[] {
    return BaseDataManager.getInstance().isBS() ? this.bsGoldRange : this.fsGoldRange;
  }
}
